...
def get_random_update(time_end, possible_dst_ids, stay_keenness, last_end=None):
    ...
    time_spent = (0.5+stay_keenness/10)*max(np.random.normal(7000, 2000),1000)
    if last_end is None:
...
def get_adaptive_update(time_end, possible_dst_ids, stay_keenness, weights, last_end=None):
    # on function call, a bot player will randomly goto the library somewhere.
    # the probability of this library will be determined by our adaptive model
    time_spent = (0.5+stay_keenness/10)*max(np.random.normal(7000, 2000),1000)
    if last_end is None:
        time_start = max(time_end-time_spent, 1)
    else:
    ...
    # the logic we choose for destination    
    dest_id = np.random.choice(possible_dst_ids, p=weights)
